...
import { IModelConfig } from './interfaces/IModelConfig'
import { IDatabaseRecord } from './interfaces/IDatabaseRecord'
...
let connection: IConnectionInfo = { hostname: "", username: "", password: "", database: "" };

//Prompt user for database connection info
function getConnectionInfo(): void {
...
async function connect(connectionInfo: IConnectionInfo) {
    try {
...
//Query database
async function query(sqlConnectionPool: any): Promise<void> {
    const table: string = prompt("Enter the name of the table or view you would like to generate a model for: ");
    const result: IQueryResult = await sql.query(`SELECT c.TABLE_NAME,c.COLUMN_NAME,c.DATA_TYPE, c.IS_NULLABLE, CASE WHEN p.COLUMN_NAME = c.COLUMN_NAME THEN 'true' ELSE 'false' END  AS isPk FROM INFORMATION_SCHEMA.COLUMNS c LEFT JOIN INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE p ON p.COLUMN_NAME = c.COLUMN_NAME WHERE c.TABLE_NAME = '${table}'`);
    ...
    if (result.rowsAffected > 0) {
        ...
        result.recordset.forEach((record: IDatabaseRecord) => {
        ...
        if (fs.existsSync("./models")) {
            ...
            fs.mkdir('./models', { recursive: true }, (err: string) => {
                if (err) console.log(err)
                else writeFile(modelConfig, table);
            });
        }
...
function writeFile(modelConfig: IModelConfig, table: string) : void {
    fs.writeFile("./models/" + table.toLowerCase() + ".json", JSON.stringify(modelConfig), function (err: string) {
        if (err) console.log(err);
        else console.log('Successfully created model ' + table.toLowerCase() + ".json!");
    });
}

//Convert SQL Server datatypes to Loopback datatypes
function parseDataType(type: string): string {
